...
RomStart:	dc.l	$00FF0200,	EntryPoint,	ErrorTrapper,	ErrorTrapper
		dc.l	ErrorTrapper,	ErrorTrapper,	ErrorTrapper,	ErrorTrapper
		dc.l	ErrorTrapper,	$FFFFC81E,	$FFFFC812,	$FFFFC818
		dc.l	ErrorTrapper,	ErrorTrapper,	ErrorTrapper,	ErrorTrapper
		dc.l	ErrorTrapper,	ErrorTrapper,	ErrorTrapper,	ErrorTrapper
		dc.l	ErrorTrapper,	ErrorTrapper,	ErrorTrapper,	ErrorTrapper
		dc.l	$FFFFC824,	ErrorTrapper,	$FFFFC82A,	ErrorTrapper
		dc.l	$FFFFC836,	ErrorTrapper,	$FFFFC830,	ErrorTrapper
		dc.l	ErrorTrapper,	ErrorTrapper,	ErrorTrapper,	ErrorTrapper
		dc.l	ErrorTrapper,	ErrorTrapper,	ErrorTrapper,	ErrorTrapper
		dc.l	ErrorTrapper,	ErrorTrapper,	ErrorTrapper,	ErrorTrapper
		dc.l	ErrorTrapper,	ErrorTrapper,	ErrorTrapper,	ErrorTrapper
...
PAL_Unknown_2:	dc.w	$0A0C,$0A06,$0E0E,$0A0E,$0E06,$0000,$0000,$020E
		dc.w	$0A0E,$0C0A,$0806,$0602,$0E0C,$0E0E,$0C00,$0E00
		dc.w	$080C,$0806,$0E08,$0206,$0202,$0000,$0000,$020E
		dc.w	$080E,$0A06,$0402,$0000,$0E0E,$0E08,$0800,$0E00
		dc.w	$080A,$0602,$0E0C,$0A08,$0802,$0000,$0000,$020E
		dc.w	$080E,$0C08,$0806,$0402,$0C0C,$0A08,$0600,$0E00
		dc.w	$0008,$040C,$0E0E,$060A,$0A04,$0000,$0002,$040C
		dc.w	$000E,$0604,$0402,$0202,$0806,$0404,$0200,$0E00
		even
...
; ===========================================================================
; ---------------------------------------------------------------------------
; Routine to decide which of the two logo effects to use
; ---------------------------------------------------------------------------

SegaEffects:						; Offset: 00006526
		tst.b	($FFFFC93C).w				; is player 1 start button held?
		bpl.s	SE_NoStart				; if not, branch
		move.w	#$0014,($FFFFD824).w			; set routine counter to 14 (Last routine)

SE_NoStart:						; Offset: 00006532
		move.w	($FFFFFAC8).w,d0			; load Sega logo effect number
		jmp	SE_Routines(pc,d0.w)			; jump to correct effect branch

; ===========================================================================
; ---------------------------------------------------------------------------
SE_Routines:						; Offset: 0000653A
		bra.w	SegaMainEffect				; Main effect used on hard reset
		bra.w	SegaScrollEffect			; Scroll effect used sometimes on soft reset
; ---------------------------------------------------------------------------
; ===========================================================================
; ---------------------------------------------------------------------------
...
SegaPaletteStart:					; Offset: 00006542
		tst.b	($FFFFC93C).w				; has player 1 held start?
		bpl.s	SPS_NoStart				; if not, branch
		move.w	#$0014,($FFFFD824).w			; set routine counter to 14

SPS_NoStart:						; Offset: 0000654E
		subq.w	#$01,($FFFFFAC4).w			; decrease effect counter
		bne.s	MultiReturn				; if not null, branch
		moveq	#$3F,d0					; set number of columns to dump
		moveq	#$3F,d1					; set number of rows to dump
		moveq	#$00,d2					; set value to dump
		move.w	($FFFFD816).w,d3			; set V-Ram location to dump to
		...
		move.w	($FFFFD818).w,d3			; set V-Ram location to dump to
		jsr	MapScreenSingle				; dump the value to V-Ram (Clear plane B)
		move.w	#$00,($FFFFFAC4).w			; clear effect counter
...
SegaPaletteCycle:					; Offset: 00006588
		tst.b	($FFFFC93C).w				; has player 1 held start?
		bpl.s	SPC_NoStart				; if not, branch
		move.w	#$0014,($FFFFD824).w			; set routine counter to 14

SPC_NoStart:						; Offset: 00006594
		lea	($FFFFD3E8).w,a0			; load palette address (starting from 3rd colour) to a0
		move.w	($FFFFFAC4).w,d0			; load effect counter to d0
		...
		move.w	#$0EEE,(a0)				; save white to colour palette
		addq.w	#$01,($FFFFFAC4).w			; increase effect counter
		cmpi.w	#$000C,($FFFFFAC4).w			; has effect counter reached C?
		bne.w	MultiReturn				; if not, branch
		move.w	#$0040,($FFFFFAC4).w			; set effect counter to 40
		addq.w	#$04,($FFFFD824).w			; increase routine counter
		rts						; return

; ===========================================================================
; ---------------------------------------------------------------------------
; Sega Screen Palette fading/finishing routine
; ---------------------------------------------------------------------------

SegaPaletteFade:					; Offset: 000065C6
		tst.b	($FFFFC93C).w				; has player 1 held start?
		bpl.s	SPF_NoStart				; if not, branch
		move.w	#$0014,($FFFFD824).w			; set routine counter to 14

SPF_NoStart:						; Offset: 000065D2
		subq.w	#$01,($FFFFFAC4).w			; decrease effect counter
...
SEGA_Dumpx3Maps:					; Offset: 0000662A
		lea	($C00000).l,a3				; load VDP data port to a3
		movea.l	a4,a5					; load starting position of mappings to a5

SD3M_DumpVert:						; Offset: 00006632
		movea.l	a5,a6					; load starting position of mappings to a6
		adda.w	($FFFFD816).w,a6			; add V-Ram plane A
		subq.w	#$06,a6					; decrease by 6
		jsr	(VRam_To_MC68).l			; convert the V-Ram location for VDP port's use
		move	#$2700,sr				; set the status register (disable interrupts)
		move.l	d0,$04(a3)				; set VDP to V-Ram write mode with location
		move.w	d6,d5					; load repeat times for horizontal dumping
		move.w	#$8100,d0				; load starting tile

SD3M_DumpHoriFG:					; Offset: 0000664E
		move.b	(a0)+,d0				; load map ID to d0 (d0 = 81??) (?? = ID)
		move.w	d0,(a3)					; dump to V-Ram
		move.b	(a0)+,d0				; load map ID
		move.w	d0,(a3)					; dump to V-Ram
		move.b	(a0)+,d0				; load map ID
		move.w	d0,(a3)					; dump to V-Ram
		move.b	(a0)+,d0				; load map ID
		move.w	d0,(a3)					; dump to V-Ram
		dbf	d5,SD3M_DumpHoriFG			; repeat for the number of horizontal tiles set
		movea.l	a5,a6					; load starting position of mappings to a6 (same as last time)
		adda.w	($FFFFD818).w,a6			; add V-Ram plane B
		subq.w	#$06,a6					; decrease by 6
		jsr	(VRam_To_MC68).l			; convert the V-Ram location for VDP port's use
		move.l	d0,$04(a3)				; set VDP to V-Ram write mode with location
		move.w	d6,d5					; load repeat times for horizontal dumping
...
SD3M_DumpHoriBG:					; Offset: 0000667A
		move.b	(a0)+,d0				; load map ID to d0 (d0 = 81??) (?? = ID)
		move.w	d0,(a3)					; dump to V-Ram
		move.b	(a0)+,d0				; load map ID
		move.w	d0,(a3)					; dump to V-Ram
		...
		move.b	(a0)+,d0				; load map ID
		move.w	d0,(a3)					; dump to V-Ram
		dbf	d5,SD3M_DumpHoriBG			; repeat for the number of horizontal tiles set
		move	#$2300,sr				; set the status register (enable interrupts)
		adda.w	d4,a5					; increase V-Ram value to another line (Normally next line 0080)
		dbf	d7,SD3M_DumpVert			; repeat for the number of vertical tiles set
		rts						; return

; ===========================================================================
; ---------------------------------------------------------------------------
; Subroutine to map the larger SEGA letters to a map plane (x4 versions)
; ---------------------------------------------------------------------------

SEGA_Dumpx4Maps:					; Offset: 0000669A
		movea.l	a4,a6					; load starting position of mappings to a6
		adda.w	($FFFFD816).w,a6			; add V-Ram plane A
		subq.w	#$08,a6					; decrease by 8

SD4M_DumpAlt:						; Offset: 000066A2
		move.w	#($80F0>>$04),d2			; starting tile for large SEGA letters
		lea	($C00000).l,a3				; load VDP data port to a3

SD4M_DumpVert:						; Offset: 000066AC
		jsr	(VRam_To_MC68).l			; convert the V-Ram location for VDP port's use
		move	#$2700,sr				; set the status register (disable interrupts)
		move.l	d0,$04(a3)				; set VDP to V-Ram write mode
		move.w	d6,d5					; load repeat times for horizontal dumping
		moveq	#$00,d0					; clear d0
		move.l	d0,(a3)					; clear first 4 tiles (Shifts the letter right a bit)
		move.l	d0,(a3)					; ''

SD4M_DumpHori:						; Offset: 000066C2
		move.l	(a0)+,d1				; load mappings
		move.l	d1,d0					; copy to d0
		move.w	d2,d0					; load starting tile ID
		rol.l	#$04,d0					; get 7th nybble on the end
		move.w	d0,(a3)					; set tile map in V-Ram
		rol.l	#$04,d1					; prepare next nybble
		move.l	d1,d0					; load mapping ID
		move.w	d2,d0					; load starting tile ID
		rol.l	#$04,d0					; get 6th nybble on the end
		move.w	d0,(a3)					; set tile map in V-Ram
		rol.l	#$04,d1					; prepare next nybble
		move.l	d1,d0					; load mapping ID
		move.w	d2,d0					; load starting tile ID
		rol.l	#$04,d0					; get 5th nybble on the end
		move.w	d0,(a3)					; set tile map in V-Ram
		rol.l	#$04,d1					; prepare next nybble
		move.l	d1,d0					; load mapping ID
		move.w	d2,d0					; load starting tile ID
		rol.l	#$04,d0					; get 4th nybble on the end
		move.w	d0,(a3)					; set tile map in V-Ram
		rol.l	#$04,d1					; prepare next nybble
		move.l	d1,d0					; load mapping ID
		move.w	d2,d0					; load starting tile ID
		rol.l	#$04,d0					; get 3rd nybble on the end
		move.w	d0,(a3)					; set tile map in V-Ram
		rol.l	#$04,d1					; prepare next nybble
		move.l	d1,d0					; load mapping ID
		move.w	d2,d0					; load starting tile ID
		rol.l	#$04,d0					; get 2nd nybble on the end
		move.w	d0,(a3)					; set tile map in V-Ram
		rol.l	#$04,d1					; prepare next nybble
		move.l	d1,d0					; load mapping ID
		move.w	d2,d0					; load starting tile ID
		rol.l	#$04,d0					; get 1st nybble on the end
		move.w	d0,(a3)					; set tile map in V-Ram
		rol.l	#$04,d1					; prepare next nybble
		move.l	d1,d0					; load mapping ID
		move.w	d2,d0					; load starting tile ID
		rol.l	#$04,d0					; get 0th nybble on the end
		move.w	d0,(a3)					; set tile map in V-Ram
		dbf	d5,SD4M_DumpHori			; repeat for the number of horizontal tiles set
		moveq	#$00,d0					; clear d0
		move.l	d0,(a3)					; clear first 4 tiles (for Shifting the letter right a bit)
		move.l	d0,(a3)					; ''
		move	#$2300,sr				; set the status register (enable interrupts)
		adda.w	d4,a6					; increase V-Ram value to another line again (Normally next line 0080)
		dbf	d7,SD4M_DumpVert			; repeat for the number of vertical tiles set
		rts						; return

; ===========================================================================
; ---------------------------------------------------------------------------
; Subroutine to map the largest SEGA letters to a map plane (x5 versions)
; ---------------------------------------------------------------------------

SEGA_Dumpx5Maps:					; Offset: 00006728
		move.w	#($80F0>>$04),d2			; starting tile for large SEGA letters
		lea	($C00000).l,a3				; load VDP data port to a3
		movea.l	a4,a6					; load starting position of mappings to a6
		adda.w	($FFFFD816).w,a6			; add V-Ram plane A

SD5M_DumpVert:						; Offset: 00006738
		jsr	(VRam_To_MC68).l			; convert the V-Ram location for VDP port's use
		move	#$2700,sr				; set the status register (disable interrupts)
		move.l	d0,$04(a3)				; set VDP to V-Ram write mode with location
		movea.l	a0,a1					; load start of mappings
		move.w	d6,d5					; load repeat times for horizontal dumping

SD5M_DumpHori1:						; Offset: 0000674A
		move.l	(a1)+,d1				; load mappings
		move.l	d1,d0					; copy to d0
		move.w	d2,d0					; load starting tile ID
		rol.l	#$04,d0					; get 7th nybble on the end
		move.w	d0,(a3)					; set tile map twice (for x2) in V-Ram
		move.w	d0,(a3)					; ''
		rol.l	#$04,d1					; prepare next nybble
		move.l	d1,d0					; load mapping ID
		move.w	d2,d0					; load starting tile ID
		...
		move.w	d0,(a3)					; set tile map twice (for x2) in V-Ram
		move.w	d0,(a3)					; ''
		dbf	d5,SD5M_DumpHori1			; repeat for the number of horizontal tiles set

; ---------------------------------------------------------------------------
; this process is repeated twice as it's 2 times to size
; ---------------------------------------------------------------------------

		adda.w	d4,a6					; increase V-Ram value to another line (Normally next line 0080)
		jsr	(VRam_To_MC68).l			; convert the V-Ram location for VDP port's use
		move.l	d0,4(a3)				; set VDP to V-Ram write mode with location
		move.w	d6,d5					; load repeat times for horizontal dumping

SD5M_DumpHori2:						; Offset: 000067BC
		move.l	(a0)+,d1				; load same mappings as last time
		move.l	d1,d0					; copy to d0
		move.w	d2,d0					; load starting tile ID
		rol.l	#$04,d0					; get 7th nybble on the end
		move.w	d0,(a3)					; set tile map twice (for x2) in V-Ram
		...
		move.w	d0,(a3)					; set tile map twice (for x2) in V-Ram
		move.w	d0,(a3)					; ''
		dbf	d5,SD5M_DumpHori2			; repeat for the number of horizontal tiles set
		move	#$2300,sr				; set the status register (enable interrupts)
		adda.w	d4,a6					; increase V-Ram value to another line again (Normally next line 0080)
		dbf	d7,SD5M_DumpVert			; repeat for the number of Vertical tiles set
		rts						; return

; ===========================================================================
; ---------------------------------------------------------------------------
; Subroutine to create large pixel tiles, decompress Sega logo art, and map
; ---------------------------------------------------------------------------
; WARNING: This is pretty complicated, so brace yourself
; ---------------------------------------------------------------------------

SEGA_LoadArt:						; Offset: 0000682C
		lea	($C00000).l,a3				; load VDP data port to a3
		moveq	#$0F,d7					; set repeat times (10 tiles)
		move	#$2700,sr				; set the status register (disable interrupts)
		move.l	#$5E000000,$04(a3)			; set VDP to VRam write mode with location
		moveq	#$00,d0					; clear d0 (resetting to 00000000)

; ---------------------------------------------------------------------------
; Largest Map Tiles (x4) & (x5)
; ---------------------------------------------------------------------------
; This part dumps 10 tiles, each one set to 1 plain colour (they represent a
; pixel each) for the largest SEGA letters.
; ---------------------------------------------------------------------------

SLA_LargestTiles:					; Offset: 00006842
		move.l	d0,(a3)					; save 1st 8 pixels
		move.l	d0,(a3)					; save 2nd 8 pixels
		move.l	d0,(a3)					; save 3rd 8 pixels
		move.l	d0,(a3)					; save 4th 8 pixels
		move.l	d0,(a3)					; save 5th 8 pixels
		move.l	d0,(a3)					; save 6th 8 pixels
		move.l	d0,(a3)					; save 7th 8 pixels
		move.l	d0,(a3)					; save last 8 pixels
		addi.l	#$11111111,d0				; increase all nybbles by 1
		dbf	d7,SLA_LargestTiles			; repeat 10 times

; ---------------------------------------------------------------------------
; Second Largest Map Tiles (x3)
; ---------------------------------------------------------------------------
; This part dumps 100 tiles, each one set to 2 plain colours in top corners
; (they represent 2 pixels each for the medium sized SEGA letters.
; ---------------------------------------------------------------------------


		moveq	#$00,d2					; clear d2 (resetting to 00000000)
		moveq	#$0F,d7					; set d7 repeat times (10 times)

SLA_LargeTiles:						; Offset: 00006860
		moveq	#$00,d1					; clear d1 (resetting to 00000000)
		moveq	#$0F,d6					; set d6 repeat times (10 times (10x10 = 100)

SLA_LT_Single:						; Offset: 00006864
		move.l	d2,d0					; copy value 2 to d0
		move.w	d1,d0					; copy value 1 to d0
		move.l	d0,(a3)					; save to V-Ram
		move.l	d0,(a3)					; ''
		move.l	d0,(a3)					; ''
		move.l	d0,(a3)					; ''
		moveq	#$00,d0					; clear d0
		...
		dbf	d6,SLA_LT_Single			; repeat 10 times
		addi.l	#$11110000,d2				; increase all nybbles in value 2 by 1
		dbf	d7,SLA_LargeTiles			; repeat 10 times

; ---------------------------------------------------------------------------
; Smallest Sprites (x1)
; ---------------------------------------------------------------------------
; This part decompresses the SEGA art and converts it correctly for the small
; SEGA logo sprites (As the art isn't in the VDP's sprite shape format, it
; needs converting).
; ---------------------------------------------------------------------------
;  _ _ _ _
; |1|2|3|4|	; shape of the sprite for each letter.
;  _ _ _ _
; |1|2|3|4|
;  _ _ _ _
; |1|2|3|4|
...
; ---------------------------------------------------------------------------
		...
		lea	($FF0200).l,a1				; load Ram location to dump, to a1
		...
		move.l	#$48000000,(a1)+			; prepare VDP V-Ram write mode location
		move.w	#$0040,(a1)+				; ...then prepare the repeat times
		moveq	#$03,d6					; set repeat times

SLA_DumpLetter:						; Offset: 000068B8
		moveq	#$1F,d7					; set repeat times for one letter

SLA_DumpSpriteRows:					; Offset: 000068BA
		move.l	(a0)+,(a1)+				; dump tile row "1"
		move.l	(a0)+,$7C(a1)				; dump tile row "2"
		move.l	(a0)+,$FC(a1)				; dump tile row "3"
		move.l	(a0)+,$17C(a1)				; dump tile row "4"
		dbf	d7,SLA_DumpSpriteRows			; repeat until tile rows "1" to "4" of this letter, have been dumped
		lea	$180(a1),a1				; advance to next letter's location
		dbf	d6,SLA_DumpLetter			; repeat until all 4 letter's art have been dumped correctly for sprites
		lea	($FF0A00).l,a1				; reload location of SEGA sprite art
		jsr	(SegaToVDP).l				; ..and dump to V-Ram

; ---------------------------------------------------------------------------
; Second Smallest Sprites (x2)
; ---------------------------------------------------------------------------
; This part expands the size of the Sega art by 2
; so for example: 0123 becomes 00112233
;			       00112233
; ---------------------------------------------------------------------------

		lea	($FF0200).l,a0				; load dumped art location to a0
		lea	($FF0A00).l,a1				; load second art location to a1
		moveq	#$7F,d6					; set repeat times (number of tiles x 2)

SLA_Expandx2:						; Offset: 000068EE
		moveq	#$07,d7					; set repeat times for two tiles

SLA_Expx2_Single:					; Offset: 000068F0
		move.w	(a0)+,d0				; load first 4 pixels
		move.w	d0,d1					; copy to d1
		move.w	d0,-(sp)				; move to the stack
		move.b	(sp)+,d0				; get the left byte only
		ror.b	#4,d0					; swap nybbles
		ror.w	#4,d0					; roll right a nybble
		swap	d0					; swap words
		move.w	d1,d0					; reload first 4 pixels
		lsl.w	#8,d0					; send right byte to the left
		move.b	d1,d0					; load the right byte
		ror.b	#4,d0					; swap nybbles
		ror.w	#4,d0					; roll right a nybble
		move.l	d0,(a1)+				; save top half of map tiles (top left/top right)
		move.l	d0,$1C(a1)				; save bottom half of map tiles (bottom left/bottom right)
		dbf	d7,SLA_Expx2_Single			; repeat til two tiles are done
		lea	$20(a1),a1				; advance to the next two tile spaces
		dbf	d6,SLA_Expandx2				; repeat for 100 tiles

; ---------------------------------------------------------------------------
; ..then converts them correctly for the second small SEGA sprites.
; ---------------------------------------------------------------------------
;  _ _ _ _   _ _ _ _
; |1|2|3|4| |5|6|7|8|	; shape of the two sprites for each letter.
;  _ _ _ _   _ _ _ _
; |1|2|3|4| |5|6|7|8|
;  _ _ _ _   _ _ _ _
; |1|2|3|4| |5|6|7|8|
...
; ---------------------------------------------------------------------------
		...
		lea	($FF0A00).l,a0				; load second art location to a0
		lea	($FF2A00).l,a1				; load third art location to a1
		move.l	#$40000001,(a1)+			; prepare VDP V-Ram location
		move.w	#$0100,(a1)+				; ...and repeat times (100 tiles)
		moveq	#$07,d6					; set repeat times

SLA_ConvSprites2:					; Offset: 00006932
		moveq	#$1F,d7					; set repeat tiles

SLA_CoSp2_Single:					; Offset: 00006934
		move.l	(a0)+,(a1)+				; dump tile row "1"
		move.l	(a0)+,$7C(a1)				; dump tile row "2"
		move.l	(a0)+,$FC(a1)				; dump tile row "3"
		move.l	(a0)+,$17C(a1)				; dump tile row "4"
		move.l	(a0)+,$1FC(a1)				; dump tile row "5"
		move.l	(a0)+,$27C(a1)				; dump tile row "6"
		move.l	(a0)+,$2FC(a1)				; dump tile row "7"
		move.l	(a0)+,$37C(a1)				; dump tile row "8"
		dbf	d7,SLA_CoSp2_Single			; repeat until tile rows "1" to "8" of this letter, have been dumped
		lea	$380(a1),a1				; advance to next letter's location
		dbf	d6,SLA_ConvSprites2			; repeat til all four letters are done
		lea	($FF2A00).l,a1				; reload location of SEGA sprite art
		jmp	(SegaToVDP).l				; ..and dump to V-Ram
...
SegaMainEffect:						; Offset: 0000696A
		subq.w	#$01,($FFFFFAC6).w			; decrease one from delay counter
		bne.w	MultiReturn				; if it's not null, branch
		addq.w	#$04,($FFFFFAC4).w			; increase effect counter
		move.w	($FFFFFAC4).w,d0			; load effect counter

SME_Routines:						; Offset: 0000697A
		jmp	SME_Routines(pc,d0.w)			; jump to correct routine based on the counter

; ===========================================================================
; ---------------------------------------------------------------------------
; "SEGA" logo effect routines
; ---------------------------------------------------------------------------
		bra.w	MapSega_Sx5				; "S" Largest Letter (Map x5)
		bra.w	MapSega_Sx4				; "S" Larger Letter (Map x4)
		bra.w	MapSega_Sx3				; "S" Large Letter (Map x3)
		bra.w	MapSega_Sx1				; "S" Smallest Letter (Sprite x1)
		bra.w	MapSega_Sx2				; "S" Small Letter (Sprite x2)
		bra.w	MapSega_Ex5				; "E" Largest Letter (Map x5)
		bra.w	MapSega_Ex4				; "E" Larger Letter (Map x4)
		bra.w	MapSega_Ex3				; "E" Large Letter (Map x3)
		bra.w	MapSega_SEx1				; "SE" Smallest Letters (Sprites x1)
		bra.w	MapSega_Ex2				; "E" Small Letter (Sprite x2)
		bra.w	MapSega_Gx5				; "G" Largest Letter (Map x5)
		bra.w	MapSega_Gx4				; "G" Larger Letter (Map x4)
		bra.w	MapSega_Gx3				; "G" Large Letter (Map x3)
		bra.w	MapSega_SEGx1				; "SEG" Smallest Letters (Sprites x1)
		bra.w	MapSega_Gx2				; "G" Small Letter (Sprite x2)
		bra.w	MapSega_Ax5				; "A" Largest Letter (Map x5)
		bra.w	MapSega_Ax4				; "A" Larger Letter (Map x4)
		bra.w	MapSega_Ax3				; "A" Large Letter (Map x3)
		bra.w	MapSega_SEGAx1				; "SEGA" Smallest Letters (Sprites x1)
		bra.w	MapSega_Ax2				; "A" Small Letter (Sprite x2)
		bra.w	MapSega_Final				; "SEGA" final spriting and finish
; ---------------------------------------------------------------------------
; ===========================================================================
; ---------------------------------------------------------------------------
; "S" Largest Letter (Map x5)
; ---------------------------------------------------------------------------

MapSega_Sx5:						; Offset: 000069D2
		move.w	#$8164,($C00004).l			; [Display On];[V-Interrupt On];[DMA Off];[V 28 Cell PAL/NTSC Mode];
		move.w	#$8164,($FFFFC9BA).w			; ...save to VDP register 01 storage too
		bsr.w	RegionTMCheck				; hide the "TM" on the SEGA logo if region is JAPAN
		lea	($FF0280).l,a0				; load location of mappings for "S"
		bra.s	MapSega_PrepMapsx5			; continue

; ---------------------------------------------------------------------------
; "E" Largest Letter (Map x5)
; ---------------------------------------------------------------------------

MapSega_Ex5:						; Offset: 000069EC
		lea	($FF0480).l,a0				; load location of mappings for "E"
		bra.s	MapSega_PrepMapsx5			; continue

; ---------------------------------------------------------------------------
; "G" Largest Letter (Map x5)
; ---------------------------------------------------------------------------

MapSega_Gx5:						; Offset: 000069F4
		lea	($FF0680).l,a0				; load location of mappings for "G"
		bra.s	MapSega_PrepMapsx5			; continue

; ---------------------------------------------------------------------------
; "A" Largest Letter (Map x5)
; ---------------------------------------------------------------------------

MapSega_Ax5:						; Offset: 000069FC
		lea	($FF0880).l,a0				; load location of mappings for "A"
								; Continues...

; ---------------------------------------------------------------------------
; Routine to prepare the ' "?" Largest Letter (Map x5) ' for dumping to the
; map planes
; ---------------------------------------------------------------------------

MapSega_PrepMapsx5:					; Offset: 00006A02
		move.w	#$0008,($FFFFFAC6).w			; set delay counter to 8
		lea	($FFFFD164).w,a1			; load sprite table address to a1
		clr.l	(a1)+					; clear half of first object (sets priority 00)
		move.w	#$FFA0,($FFFFCA5E).w			; set FG X scroll position
		move.w	#$FFA0,($FFFFCA60).w			; set BG X scroll position
		move.w	#$0018,($FFFFCDDE).w			; set FG Y scroll position
		move.w	#$0014,($FFFFCDE0).w			; set BG Y scroll position
		lea	($00000000).w,a4			; clear a4
		move.w	#$0080,d4				; map lines to increase by (one line)
		moveq	#$03,d6					; set number of horizontal tiles worth of maps to dump
		moveq	#$0F,d7					; set number of vertical tiles worth of maps to dump
		bra.w	SEGA_Dumpx5Maps				; dump the x5 letter to screen

; ===========================================================================
; ---------------------------------------------------------------------------
; "S" Larger Letter (Map x4)
; ---------------------------------------------------------------------------

MapSega_Sx4:						; Offset: 00006A36
		lea	($FF0200).l,a0				; load location of mappings for "S"
		bra.s	MapSega_PrepMapsx4			; continue

; ---------------------------------------------------------------------------
; "E" Larger Letter (Map x4)
; ---------------------------------------------------------------------------

MapSega_Ex4:						; Offset: 00006A3E
		lea	($FF0400).l,a0				; load location of mappings for "E"
		bra.s	MapSega_PrepMapsx4			; continue

; ---------------------------------------------------------------------------
; "G" Larger Letter (Map x4)
; ---------------------------------------------------------------------------

MapSega_Gx4:						; Offset: 00006A46
		lea	($FF0600).l,a0				; load location of mappings for "G"
		bra.s	MapSega_PrepMapsx4			; continue

; ---------------------------------------------------------------------------
; "A" Larger Letter (Map x4)
; ---------------------------------------------------------------------------

MapSega_Ax4:						; Offset: 00006A4E
		lea	($FF0800).l,a0				; load location of mappings for "A"
								; Continues...

; ---------------------------------------------------------------------------
; Routine to prepare the ' "?" Larger Letter (Map x4) ' for dumping to the
; map planes
; ---------------------------------------------------------------------------

MapSega_PrepMapsx4:					; Offset: 00006A54
		move.w	#$0004,($FFFFFAC6).w			; set delay counter to 4
		lea	($FFFFD164).w,a1			; load sprite table address to a1
		clr.l	(a1)+					; clear half of first object (sets priority 00)
		move.w	#$FFA0,($FFFFCA5E).w			; set FG X scroll position
		move.w	#$FFA0,($FFFFCA60).w			; set BG X scroll position
		move.w	#$0018,($FFFFCDDE).w			; set FG Y scroll position
		move.w	#$0014,($FFFFCDE0).w			; set BG Y scroll position
		lea	($00000020).w,a4			; set 20 in a4 (starting position for mappings)
		move.w	#$0080,d4				; map lines to increase by (one line)
		moveq	#$03,d6					; set number of horizontal tiles worth of maps to dump
		moveq	#$1F,d7					; set number of vertical tiles worth of maps to dump
		bra.w	SEGA_Dumpx4Maps				; dump the x4 letter to screen

; ===========================================================================
; ---------------------------------------------------------------------------
; "S" Large Letter (Map x3)
; ---------------------------------------------------------------------------

MapSega_Sx3:						; Offset: 00006A88
		lea	($FF0200).l,a0				; load location of mappings for "S"
		move.w	#$FFA0,d0				; prepare X scroll position for "S"
		bra.s	MapSega_PrepMapsx3			; continue

; ---------------------------------------------------------------------------
; "E" Large Letter (Map x3)
; ---------------------------------------------------------------------------

MapSega_Ex3:						; Offset: 00006A94
		lea	($FF0400).l,a0				; load location of mappings for "E"
		move.w	#$FFB0,d0				; prepare X scroll position for "E"
		bra.s	MapSega_PrepMapsx3			; continue

; ---------------------------------------------------------------------------
; "G" Large Letter (Map x3)
; ---------------------------------------------------------------------------

MapSega_Gx3:						; Offset: 00006AA0
		lea	($FF0600).l,a0				; load location of mappings for "G"
		move.w	#$FFC0,d0				; prepare X scroll position for "G"
		bra.s	MapSega_PrepMapsx3			; continue

; ---------------------------------------------------------------------------
; "A" Large Letter (Map x3)
; ---------------------------------------------------------------------------

MapSega_Ax3:						; Offset: 00006AAC
		lea	($FF0800).l,a0				; load location of mappings for "A"
		move.w	#$FFD0,d0				; prepare X scroll position for "A"
								; Continues...

; ---------------------------------------------------------------------------
; Routine to prepare the ' "?" Large Letter (Map x3) ' for dumping to the map
; planes
; ---------------------------------------------------------------------------

MapSega_PrepMapsx3:					; Offset: 00006AB6
		move.w	#$0004,($FFFFFAC6).w			; set delay counter to 4
		lea	($FFFFD164).w,a1			; load sprite table address to a1
		clr.l	(a1)+					; clear half of first object (sets priority 00)
		move.w	d0,($FFFFCA5E).w			; set FG X scroll position
		move.w	d0,($FFFFCA60).w			; set BG X scroll position
		move.w	#$0118,($FFFFCDDE).w			; set FG Y scroll position
		move.w	#$0114,($FFFFCDE0).w			; set BG Y scroll position
		lea	($00001430).w,a4			; set 1430 in a4 (starting position for mappings)
		move.w	#$0080,d4				; map lines to increase by (one line)
		moveq	#$03,d6					; set number of horizontal tiles worth of maps to dump
		moveq	#$0F,d7					; set number of vertical tiles worth of maps to dump
		bra.w	SEGA_Dumpx3Maps				; dump the x3 letter to screen

; ===========================================================================
; ---------------------------------------------------------------------------
; "S" Smallest Letter (Sprite x1)
; ---------------------------------------------------------------------------

MapSega_Sx1:						; Offset: 00006AE6
		move.w	#$0200,d7				; prepare V-Ram location for sprite "S"
		move.w	#$00E0,d0				; prepare X position for sprite "S"
		bra.s	MapSega_Dumpx2				; continue

; ---------------------------------------------------------------------------
; "SE" Smallest Letters (Sprites x1)
; ---------------------------------------------------------------------------

MapSega_SEx1:						; Offset: 00006AF0
		move.w	#$0240,d7				; prepare V-Ram location for sprites "SE"
		move.w	#$00F0,d0				; prepare X position for sprites "SE"
		bra.s	MapSega_Dumpx2				; continue

; ---------------------------------------------------------------------------
; "SEG" Smallest Letters (Sprites x1)
; ---------------------------------------------------------------------------

MapSega_SEGx1:						; Offset: 00006AFA
		move.w	#$0280,d7				; prepare V-Ram location for sprites "SEG"
		move.w	#$0110,d0				; prepare X position for sprites "SEG"
		bra.s	MapSega_Dumpx2				; continue

; ---------------------------------------------------------------------------
; "SEGA" Smallest Letters (Sprites x1)
; ---------------------------------------------------------------------------

MapSega_SEGAx1:						; Offset: 00006B04
		move.w	#$02C0,d7				; prepare V-Ram location for sprites "SEGA"
		move.w	#$0120,d0				; prepare X position for sprites "SEGA"
								; Continues...

; ---------------------------------------------------------------------------
; Routine to dump the sprites "SEGA" to the sprite table correctly
; ---------------------------------------------------------------------------

MapSega_Dumpx2:						; Offset: 00006B0C
		move.w	#$04,($FFFFFAC6).w			; set delay counter to 4
		move.w	#$00A0,($FFFFCA5E).w			; set FG X scroll position
		move.w	#$00A0,($FFFFCA60).w			; set BG X scroll position
		move.w	#$0118,($FFFFCDDE).w			; set FG Y scroll position
		move.w	#$0114,($FFFFCDE0).w			; set BG Y scroll position
		lea	($FFFFD164).w,a0			; load sprite table address to a0
		move.l	#$00C80F01,(a0)+			; set "S" sprite XXXX YYZZ (XXXX = Y position/YY shape/ZZ priority)
		move.w	d7,(a0)+				; set "S" sprite V-Ram location read
		move.w	d0,(a0)+				; set "S" sprite X position
		addi.w	#$0010,d7				; increase V-Ram location read by 10 tiles
		move.l	#$00C80F02,(a0)+			; set "E" sprite XXXX YYZZ (XXXX = Y position/YY shape/ZZ priority)
		move.w	d7,(a0)+				; set "E" sprite V-Ram location read
		addi.w	#$0020,d0				; increase X position by 20
		move.w	d0,(a0)+				; set "E" sprite X position
		addi.w	#$0010,d7				; increase V-Ram location read by 10 tiles
		move.l	#$00E80F03,(a0)+			; set "G" sprite XXXX YYZZ (XXXX = Y position/YY shape/ZZ priority)
		move.w	d7,(a0)+				; set "G" sprite V-Ram location read
		subi.w	#$0020,d0				; increase X position by 20
		move.w	d0,(a0)+				; set "G" sprite X position
		addi.w	#$0010,d7				; increase V-Ram location read by 10 tiles
		move.l	#$00E80F04,(a0)+			; set "A" sprite XXXX YYZZ (XXXX = Y position/YY shape/ZZ priority)
		move.w	d7,(a0)+				; set "A" sprite V-Ram location read
		addi.w	#$0020,d0				; increase X position by 20
		move.w	d0,(a0)					; set "A" sprite X position
		rts						; return

; ===========================================================================
; ---------------------------------------------------------------------------
; "S" Small Letter (Sprite x2)
; ---------------------------------------------------------------------------

MapSega_Sx2:						; Offset: 00006B70
		move.l	#$004000EF,d7				; prepare V-Ram location and X position for 5th sprite
		lea	($FFFFD184).w,a0			; load sprite table starting from 5th sprite slot
		bra.s	MapSega_Dumpx1				; continue

; ---------------------------------------------------------------------------
; "E" Small Letter (Sprite x2)
; ---------------------------------------------------------------------------

MapSega_Ex2:						; Offset: 00006B7C
		move.l	#$00500105,d7				; prepare V-Ram location and X position 6th sprite
		lea	($FFFFD18C).w,a0			; load sprite table starting from 6th sprite slot
		bra.s	MapSega_Dumpx1				; continue

; ---------------------------------------------------------------------------
; "G" Small Letter (Sprite x2)
; ---------------------------------------------------------------------------

MapSega_Gx2:						; Offset: 00006B88
		move.l	#$0060011B,d7				; prepare V-Ram location and X position 7th sprite
		lea	($FFFFD194).w,a0			; load sprite table starting from 8th sprite slot
		bra.s	MapSega_Dumpx1				; continue

; ---------------------------------------------------------------------------
; "A" Small Letter (Sprite x2)
; ---------------------------------------------------------------------------

MapSega_Ax2:						; Offset: 00006B94
		move.l	#$00700135,d7				; prepare V-Ram location and X position 8th sprite
		lea	($FFFFD19C).w,a0			; load sprite table starting from 9th sprite slot
								; Continues...

; ---------------------------------------------------------------------------
; Routine to dump the sprites ' "?" Small Letter (Sprite x2) ' to the sprite
; table correctly
; ---------------------------------------------------------------------------

MapSega_Dumpx1:						; Offset: 00006B9E
		move.w	#$04,($FFFFFAC6).w			; set delay counter to 4
		move.w	#$00A0,($FFFFCA5E).w			; set FG X scroll position
		move.w	#$00A0,($FFFFCA60).w			; set BG X scroll position
		move.w	#$0118,($FFFFCDDE).w			; set FG Y scroll position
		move.w	#$0114,($FFFFCDE0).w			; set BG Y scroll position
		move.w	#$00D8,(a0)+				; set Y position
		addq.l	#$02,a0					; skip the shape and priority
		move.l	d7,(a0)					; set sprite XXXX YYYY (XXXX = V-Ram location/YYYY = X position)
		lea	($FFFFD164).w,a0			; load beginning of sprite table to a0
		clr.w	(a0)					; set Y position of 1st sprite to 0
		addq.l	#$08,a0					; advance to next sprite slot
		clr.w	(a0)					; set Y position of 2nd sprite to 0
		addq.l	#$08,a0					; advance to next sprite slot
		clr.w	(a0)					; set Y position of 3rd sprite to 0
		addq.l	#$08,a0					; advance to next sprite slot
		clr.w	(a0)					; set Y position of 4th sprite to 0
		rts						; return

; ===========================================================================
; ---------------------------------------------------------------------------
; "SEGA" final spriting and finish
; ---------------------------------------------------------------------------

MapSega_Final:						; Offset: 00006BD8
		lea	($FFFFD164).w,a0			; load sprite table to a0
		move.l	$20(a0),d0				; copy 5th sprite "S" (Y pos/shape/priority) to d0
		move.b	$03(a0),d0				; load 1st sprite "S" priority (to replace 5th's)
		move.l	d0,(a0)+				; save as 1st sprite
		move.l	$20(a0),(a0)+				; copy 5th sprite "S" (V-Ram/X pos) to 1st sprite "S"
		move.l	$20(a0),d0				; copy 6th sprite "E" (Y pos/shape/priority) to d0
		move.b	$03(a0),d0				; load 2nd sprite "E" priority (to replace 6th's)
		move.l	d0,(a0)+				; save as 2nd sprite
		move.l	$20(a0),(a0)+				; copy 6th sprite "E" (V-Ram/X pos) to 2nd sprite "E"
		move.l	$20(a0),d0				; copy 7th sprite "G" (Y pos/shape/priority) to d0
		move.b	$03(a0),d0				; load 3rd sprite "G" priority (to replace 7th's)
		move.l	d0,(a0)+				; save as 3rd sprite
		move.l	$20(a0),(a0)+				; copy 7th sprite "G" (V-Ram/X pos) to 3rd sprite "G"
		move.l	$20(a0),d0				; copy 8th sprite "A" (Y pos/shape/priority) to d0
		move.b	$03(a0),d0				; load 4th sprite "A" priority (to replace 8th's)
		move.l	d0,(a0)+				; save as 4th sprite
		move.l	$20(a0),(a0)				; copy 8th sprite "A" (V-Ram/X pos) to 4th sprite "A"
		move.w	#$0010,($FFFFFAC4).w			; set SEGA effect counter to 10
		addq.w	#$04,($FFFFD824).w			; increase routine counter
		rts						; return

; ===========================================================================
; ---------------------------------------------------------------------------
; Sega Scroll Effect (works Randomly on Soft Reset)
; ---------------------------------------------------------------------------

SegaScrollEffect:					; Offset: 00006C20
		move.w	($FFFFFAC4).w,d0			; load effect counter
		jmp	SSE_Routines(pc,d0.w)			; jump to correct routine based on the counter

; ===========================================================================
; ---------------------------------------------------------------------------
SSE_Routines:						; Offset: 00006C28
		bra.w	MapScroll_Mapx4SEGA			; Mapping "SEGA" letters to the planes
		bra.w	MapScroll_MoveLeft			; Moving the horizontal scrollings left for SEGA
		bra.w	MapScroll_RemoveMask			; Removing the "A" Mask
		bra.w	MapScroll_MoveLeft			; Moving the horizontal scrollings left for SEGA
		bra.w	MapScroll_LoadSprites			; Loading the "SEGA" sprites
		bra.w	MapScroll_MoveRight			; Moving the sprites right for SEGA
		bra.w	MapScroll_Reflect			; Bouncing/Reflecting the SEGA letters when they meet the center of the screen
		bra.w	MapScroll_Final				; "SEGA" final spriting and finish
; ---------------------------------------------------------------------------
; ===========================================================================
; ---------------------------------------------------------------------------
; Mapping "SEGA" letters to the planes
; ---------------------------------------------------------------------------

MapScroll_Mapx4SEGA:					; Offset: 00006C48
		move.w	#$9003,($C00004).l			; [VDP Screen Map Size: Vertical Size 32 Cell/Horizontal Size 128 Cell];
		move.w	#$9003,($FFFFC9D8).w			; ...and store in VDP register 10 data
		lea	($C00000).l,a3				; load VDP data port to a3
		movea.w	($FFFFD816).w,a6			; load V-Ram plane A location
		lea	$BE(a6),a6				; increase by BE
		move.l	#$A0F1A0F1,d2				; prepare the black tile map ID's that are to be dumped
		move	#$2700,sr				; set the status register (disable interrupts)
		moveq	#$1F,d7					; set repeat times

MSx4S_MaskA:						; Offset: 00006C70
		jsr	(VRam_To_MC68).l			; convert the V-Ram location for VDP port's use
		move.l	d0,$04(a3)				; set VDP to V-Ram write mode (with location)
		move.l	d2,(a3)					; save map ID's to V-Ram (8 per two lines)
		move.l	d2,(a3)					; ''
		move.l	d2,(a3)					; ''
		move.l	d2,(a3)					; ''
		adda.w	#$0100,a6				; advance to next line
		dbf	d7,MSx4S_MaskA				; repeat til th left side of the screen is set using black tiles (Prevents the letter "A" from showing on the right)
		move	#$2300,sr				; set the status register (enable interrupts)
		addq.w	#$04,($FFFFFAC4).w			; increase routine counter
		move.w	#$0020,($FFFFFAC6).w			; set delay counter to 20
		move.w	#$00F8,($FFFFCA5E).w			; set FG X scroll position
		move.w	#$0018,($FFFFCDDE).w			; set FG Y scroll position
		move.w	#$00F8,($FFFFCA60).w			; set BG X scroll position
		move.w	#$0018,($FFFFCDE0).w			; set BG Y scroll position
		lea	($FF0200).l,a0				; load SEGA's "S" art/map address to a0
		movea.w	($FFFFD816).w,a6			; load V-Ram plane A location to a6
		bsr.w	MSx4S_MapLetter				; dump letter to screen (FG)
		lea	($FF0400).l,a0				; load SEGA's "S" art/map address to a0
		movea.w	($FFFFD818).w,a6			; load V-Ram plane B location to a6
		lea	$2C(a6),a6				; increase by 2C
		bsr.w	MSx4S_MapLetter				; dump letter to screen (BG)
		lea	($FF0600).l,a0				; load SEGA's "E" art/map address to a0
		movea.w	($FFFFD816).w,a6			; load V-Ram plane A location to a6
		lea	$58(a6),a6				; increase by 58
		bsr.w	MSx4S_MapLetter				; dump letter to screen (FG)
		lea	($FF0800).l,a0				; load SEGA's "E" art/map address to a0
		movea.w	($FFFFD818).w,a6			; load V-Ram plane B location to a6
		lea	$8C(a6),a6				; increase by 8C
						; Continue to..	; dump letter to screen (BG)

MSx4S_MapLetter:					; Offset: 00006CF0
		move.w	#$0100,d4				; map lines to increase by (two lines)
		moveq	#$03,d6					; set number of horizontal tiles worth of maps to dump
		moveq	#$1F,d7					; set number of vertical tiles worth of maps to dump
		bra.w	SD4M_DumpAlt				; dump x4 letter to screen

; ===========================================================================
; ---------------------------------------------------------------------------
; Moving the horizontal scrollings left for SEGA
; ---------------------------------------------------------------------------

MapScroll_MoveLeft:					; Offset: 00006CFC
		move.w	#$8164,($C00004).l			; [Display On];[V-Interrupt On];[DMA Off];[V 28 Cell PAL/NTSC Mode];
		move.w	#$8164,($FFFFC9BA).w			; ...save to VDP register 01 storage too
		subi.w	#$0010,($FFFFCA5E).w			; decrease FG X scroll position
		subi.w	#$0010,($FFFFCA60).w			; decrease BG X scroll position
		subq.w	#$01,($FFFFFAC6).w			; decrease counter
		bne.s	MSML_KeepScrolling			; if not null, branch
		addq.w	#$04,($FFFFFAC4).w			; increase sega effect counter

MSML_KeepScrolling:					; Offset: 00006D20
		rts						; return
...
MapScroll_RemoveMask:					; Offset: 00006D22
		lea	($C00000).l,a3				; load VDP data port to a3
		moveq	#$00,d2					; clear d2
		movea.w	($FFFFD816).w,a6			; load V-Ram plane A location to a6
		lea	$BE(a6),a6				; increase by BE
...
MSRM_PresentA:						; Offset: 00006D38
		...
		move.l	d2,(a3)					; ''
		adda.w	#$0100,a6				; advance to next line
		dbf	d7,MSRM_PresentA			; repeat til the left masking is removed for the "A"
		move	#$2300,sr				; set the status register (enable interrupts)
		subi.w	#$0010,($FFFFCA5E).w			; decrease FG X scroll position
		subi.w	#$0010,($FFFFCA60).w			; decrease BG X scroll position
		addq.w	#$04,($FFFFFAC4).w			; increase sega effect counter
		move.w	#$0020,($FFFFFAC6).w			; reset delay counter to 20 again
		rts						; return

; ===========================================================================
; ---------------------------------------------------------------------------
; Loading the "SEGA" sprites
; ---------------------------------------------------------------------------

MapScroll_LoadSprites:					; Offset: 00006D6E
		bsr.w	RegionTMCheck				; hide the "TM" on the SEGA logo if region is JAPAN
		move	#$2700,sr				; set the status register (disable interrupts)
		moveq	#$3F,d0					; set number of columns to dump
		moveq	#$3F,d1					; set number of rows to dump
		...
		jsr	MapScreenSingle				; dump the value to V-Ram (Clear plane B)
		move	#$2300,sr				; set the status register (enable interrupts)
		lea	(MSLS_SpriteData).l,a0			; load sprite data
		lea	($FFFFD164).w,a1			; load sprite table to a1
		move.l	(a0)+,(a1)+				; dump sprite data to the sprite table
		move.l	(a0)+,(a1)+				; ''
		move.l	(a0)+,(a1)+				; ''
		move.l	(a0)+,(a1)+				; ''
		move.l	(a0)+,(a1)+				; ''
...
; ===========================================================================
; ---------------------------------------------------------------------------
; Sprite data:	dc.l	$YYYYSSPP,$VVVVXXXX
; ---------------------------------------------------------------------------
; Format: (...is the same as the normal VDP format)
; YYYY = Y position of sprite
; SS = shape of sprite
; PP = priority of sprite
; VVVV = V-Ram location read of sprite
; XXXX = X position of sprite
; ---------------------------------------------------------------------------
MSLS_SpriteData:					; Offset: 00006DBC
		dc.l	$00D80F01,$0040FFEF			; "S" sprite
		dc.l	$00D80F02,$00500005			; "E" sprite
		dc.l	$00D80F03,$0060001B			; "G" sprite
		dc.l	$00D80F00,$00700035			; "A" sprite
		even
; ---------------------------------------------------------------------------
; ===========================================================================
; ---------------------------------------------------------------------------
; Moving the sprites right for SEGA
; ---------------------------------------------------------------------------

MapScroll_MoveRight:					; Offset: 00006DDC
		addq.w	#$08,($FFFFD16A).w			; increase "S" sprite's X position by 8 pixels
		addq.w	#$08,($FFFFD172).w			; increase "E" sprite's X position by 8 pixels
		addq.w	#$08,($FFFFD17A).w			; increase "G" sprite's X position by 8 pixels
		addq.w	#$08,($FFFFD182).w			; increase "A" sprite's X position by 8 pixels
		subq.w	#$01,($FFFFFAC6).w			; decrease delay counter
		bne.s	MSMR_KeepScrolling			; if not null, branch
		addq.w	#$04,($FFFFFAC4).w			; increase sega effect counter
		move.w	#$0021,($FFFFFAC6).w			; set delay counter to 21

MSMR_KeepScrolling:					; Offset: 00006DFC
		rts						; return

; ===========================================================================
; ---------------------------------------------------------------------------
; Bouncing/Reflecting the SEGA letters when they meet the center of the screen
; ---------------------------------------------------------------------------

MapScroll_Reflect:					; Offset: 00006DFE
		subq.w	#$01,($FFFFFAC6).w			; decrease delay counter
		beq.s	MSR_Finish				; if null, branch
		move.w	($FFFFFAC6).w,d0			; load delay counter
		move.w	d0,d1					; copy to d1
		andi.w	#$0003,d1				; get only within 3
		bne.s	MSR_WaitCounter				; if not within a multiple of 4, branch
		andi.w	#$001C,d0				; get only within 1C in multiples of 4
		lea	(MSR_ReflectData).l,a0			; load reflect data's location to a0
		adda.w	d0,a0					; add counter to the reflect location
		lea	($FFFFD16A).w,a1			; load sprite counter starting from 1st sprite's X position
		move.b	(a0)+,d0				; load 1st position
		ext.w	d0					; set as word signed
		add.w	d0,(a1)					; add to 1st sprite's current X position
		move.b	(a0)+,d0				; load 2nd position
		ext.w	d0					; set as word signed
		add.w	d0,$08(a1)				; add to 2nd sprite's current X position
		move.b	(a0)+,d0				; load 3rd position
		ext.w	d0					; set as word signed
		add.w	d0,$10(a1)				; add to 3rd sprite's current X position
		move.b	(a0)+,d0				; load 4th position
		ext.w	d0					; set as word signed
		add.w	d0,$18(a1)				; add to 4th sprite's current X position

MSR_WaitCounter:					; Offset: 00006E3E
		rts						; return

MSR_Finish:						; Offset: 00006E40
		addq.w	#$04,($FFFFFAC4).w			; increase sega effect counter
		rts						; return

; ===========================================================================
; ---------------------------------------------------------------------------
; X position adding data, to make the SEGA letters bounce/reflect.
; ---------------------------------------------------------------------------
; Format:	dc.b	$11,$22,$33,$44
; 11 = 1st sprites X position to add
; 22 = 2nd sprites X position to add
; 33 = 3rd sprites X position to add
; 44 = 4th sprites X position to add
; ---------------------------------------------------------------------------
MSR_ReflectData:					; Offset: 00006E46
		dc.b	$00,$00,$00,$00				; 00 delay Counter/Timeline
		dc.b	$FE,$FF,$FF,$00				; 04
		dc.b	$FE,$FE,$FF,$FF				; 08
		dc.b	$FE,$FF,$FF,$00				; 0C
		dc.b	$FE,$FE,$FF,$FF				; 10
...
MapScroll_Final:					; Offset: 00006E66
		lea	($FFFFD184).w,a0			; load sprite table starting from the 5th sprite slot
		move.l	-$20(a0),d0				; load 1st sprite's data to d0 (Y pos/Shape/Priority)
		move.b	$03(a0),d0				; load 5th sprite's priority over the 1st's
		move.l	d0,(a0)+				; save to 5th sprite
		move.l	-$20(a0),(a0)+				; load 1st sprite's data (V-Ram/X pos), save to 5th sprite
		move.l	-$20(a0),d0				; load 2nd sprite's data to d0 (Y pos/Shape/Priority)
		move.b	$03(a0),d0				; load 6th sprite's priority over the 2nd's
		move.l	d0,(a0)+				; save to 6th sprite
		move.l	-$20(a0),(a0)+				; load 2nd sprite's data (V-Ram/X pos), save to 6th sprite
		move.l	-$20(a0),d0				; load 3rd sprite's data to d0 (Y pos/Shape/Priority)
		move.b	$03(a0),d0				; load 7th sprite's priority over the 3rd's
		move.l	d0,(a0)+				; save to 7th sprite
		move.l	-$20(a0),(a0)+				; load 3rd sprite's data (V-Ram/X pos), save to 7th sprite
		move.l	-$20(a0),d0				; load 4th sprite's data to d0 (Y pos/Shape/Priority)
		move.b	$03(a0),d0				; load 8th sprite's priority over the 4th's
		move.l	d0,(a0)+				; save to 8th sprite
		move.l	-$20(a0),(a0)				; load 4th sprite's data (V-Ram/X pos), save to 8th sprite
		move.b	#$04,-$21(a0)				; set 4th sprite's priority to 4 (was 0 to set end of table)
		move.w	#$0010,($FFFFFAC4).w			; set sega effect counter to 10
		addq.w	#$04,($FFFFD824).w			; increase routine counter
		rts						; return
...
SonicIndex:                           ; DATA XREF: ROM:0000A27Eâ†‘o
                                        ; ROM:0000A284â†“o ...
                rts
; ---------------------------------------------------------------------------
...
loc_A296:                               ; DATA XREF: ROM:0000A284â†‘o
		pea	loc_A506(pc)
		bset	#0,$25(a6)
		bclr	#4,$25(a6)
		clr.l	$18(a6)
...
RO_Routines:
		...
		bra.w	locret_EBAC				; Object 08 - Red Sprints (Facing up)
...
loc_DB00:				; CODE XREF: ROM:0000DAF6j
					; ROM:0000DAFCj
		ori.w	#1,$2A(a6)
		move.l	#ARTUNC_Tails,d0 ; "\x0Eì\x0EºÌ\x0E²ª\x0E¹2\x0E»I\x0EëDî´îëì»ì»ì»\x0EË\x0EÌãì\fîÎ\x01\x1E\x1E"...
		moveq	#0,d1
		jsr	(loc_CB1C).l
...
loc_DCDC:				; CODE XREF: ROM:0000DCD2j
					; ROM:0000DCD8j
		ori.w	#1,$2A(a6)
		move.l	#ARTUNC_Tails,d1 ; "\x0Eì\x0EºÌ\x0E²ª\x0E¹2\x0E»I\x0EëDî´îëì»ì»ì»\x0EË\x0EÌãì\fîÎ\x01\x1E\x1E"...
		neg.l	d1
		moveq	#0,d0
		jsr	(loc_CB1C).l
...
; ---------------------------------------------------------------------------
AL05:		dcb.b ($0002C000+AlignValue)-AL05,$FF		; Aligned
; ---------------------------------------------------------------------------
